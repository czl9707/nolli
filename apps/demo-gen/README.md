...
4. **Windows Chrome for capture** — WSL Chromium rasterizes in software (no
   hardware Vulkan ICD), which starves the demo screencast to a few fps and
   judders the clip, so the capture scripts always launch Windows Chrome (real
   GPU) and connect over CDP. One-time setup in an elevated PowerShell on
   Windows:
