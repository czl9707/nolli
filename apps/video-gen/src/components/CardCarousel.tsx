...
export const CARD_W = 608; // focused card, 4:5 portrait
export const CARD_H = 760;
export const CARD_PITCH = Math.round(CARD_H * 0.15); // first offset step
// Each successive offset step halves — diminishing pitch, receding-stack feel.
export const PITCH_DECAY = 0.5;
// Cards beyond CARD_DEPTH are hard-culled; the VEIL_CAP darkness hides the pop
// at the cull edge, so no dissolve is needed.
export const CARD_DEPTH = 2;
// Caps <1 so receding cards read as darkened photos, never solid black boxes.
...
// Power of the per-slot ease-out snap into center.
const SNAP_EASE = 10;
const SNAP_OUT = Easing.out(Easing.poly(SNAP_EASE));

/** Fractional focused-building for a WALK-local frame: ease-out snap into
 *  center at each slot start, then hold. */
 ...
 *  Cover-only; captions live off-card in BuildingCaption. */
